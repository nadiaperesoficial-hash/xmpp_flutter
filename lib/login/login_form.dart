...
// Servidores públicos Jabber/XMPP
...
class _LoginFormState extends State<LoginForm>
  ...
  void _onRegisterPressed() {
    if (_domainController.text.trim().isEmpty) {
      setState(() => _authMessage = 'Selecione um servidor para criar conta');
      return;
    }
    ...
    return BlocConsumer<LoginBloc, LoginState>(
      ...
      builder: (context, state) {
        if (state is RegisterLoading) {
          return _buildLoading('Criando conta...');
        }
        if (state is LoginLoading) {
          return _buildLoading('Conectando...');
        }
        return _buildForm();
      },
    );
    ...
    return ListView(
      ...
      children: [
          ...
          children: [
            ...
            TextButton(
              onPressed: () => setState(() {
                ...
                _authMessage = null;
                if (!_isExtended) _isExtended = true;
              }),
              child: Text(
                'Criar conta',
                style: TextStyle(
        ...
        TextFormField(
          ...
          keyboardType: _isExtended
              ? TextInputType.text
              : TextInputType.emailAddress,
          decoration: InputDecoration(
            hintText: _isExtended ? 'usuário' : 'usuário@servidor.com',
          ...
          children: [
            Row(
              children: [
                Checkbox(
                  value: _rememberMe,
                  onChanged: (v) => _loginBloc
                      .add(RememberMePressed(rememberMeValue: v ?? false)),
                ),
                const Text('Lembrar'),
              ],
            ),
            TextButton(
              onPressed: () => _loginBloc.add(const ExtendPressed()),
              child: Text(
                _isExtended ? 'Básico' : 'Avançado',
        ...
        ElevatedButton(
          onPressed:
              _isRegisterMode ? _onRegisterPressed : _onLoginPressed,
        ...
        if (!_isRegisterMode)
          TextButton(
            onPressed: () {
              final domain = _domainController.text.trim().isNotEmpty
                  ? _domainController.text.trim()
                  : 'seu servidor';
              showDialog(
                context: context,
                builder: (_) => AlertDialog(
                  ...
                  content: Text(
                    'Para recuperar sua senha, acesse o site do servidor "$domain" '
                    'e siga as instruções de recuperação de conta.',
                  ),
                  actions: [
                    TextButton(
